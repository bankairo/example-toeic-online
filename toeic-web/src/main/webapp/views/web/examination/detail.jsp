...
<c:set var="PHOTO" value="<%=WebConstant.EXAMINATIONQUESTION_TYPE_PHOTO%>"/>
<c:set var="QUESTION_RESPONSE" value="<%=WebConstant.EXAMINATIONQUESTION_TYPE_QUESTION_RESPONSE%>"/>
<c:set var="SINGLE_PASSAGE" value="<%=WebConstant.EXAMINATIONQUESTION_TYPE_SINGLE_PASSAGE%>"/>
    ...
    <div class="grid news_desc">
        <div style="overflow: auto; height: 500px" >
            <c:forEach items="${items.listResult}" var="item">
                <c:if test="${not empty item.number}">
                    <b>${item.number}.${item.question}</b>
                </c:if>
                <c:if test="${empty item.number}">
                    <b>${item.question}</b>
                </c:if>
                <c:choose>
                    ...
                    <c:when test="${item.type == SINGLE_PASSAGE}">
                        <c:if test="${not empty item.paragraph}">
                            <p>
                                ${item.paragraph}
                            </p>
                        </c:if>
